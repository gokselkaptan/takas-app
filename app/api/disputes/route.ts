...
// Dispute türlerini Türkçe'ye çevir — GÖREV 46: Güncellenmiş kategoriler
...
// GÖREV 46: Beklenen çözüm seçenekleri
...
// POST: Sorun raporu aç — GÖREV 46: Geliştirilmiş
export async function POST(request: Request) {
  try {
    ...
    // GÖREV 46: Yeni alanları al
    const { swapRequestId, type, description, evidence, contactEmail, disputeType, expectedResolution } = await request.json()

    if (!swapRequestId) {
      return NextResponse.json({ error: 'Takas ID gerekli' }, { status: 400 })
    }

    // GÖREV 46: Güncellenmiş kategori listesi
    const validTypes = [
      'defect', 'not_as_described', 'missing_parts', 'damaged', 'wrong_item', 'no_show', 'other',
      'product_mismatch', 'product_damaged', 'product_not_delivered', 'wrong_product', 
      'valor_dispute', 'communication_issue', 'fraud_suspicion'
    ]
    ...
    // GÖREV 46: Beklenen çözüm zorunlu kontrolü
    const validResolutions = ['refund_valor', 'product_return', 'replacement', 'partial_refund', 'apology', 'other']
    if (!expectedResolution || !validResolutions.includes(expectedResolution)) {
      return NextResponse.json({ error: 'Beklenen çözüm seçimi gerekli' }, { status: 400 })
    }

    const swapRequest = await prisma.swapRequest.findUnique({
      where: { id: swapRequestId },
      include: {
        product: { select: { id: true, title: true, valorPrice: true } },
    ...
    const existingDispute = swapRequest.disputes.find((d: { status: string }) => d.status === 'open' || d.status === 'under_review' || d.status === 'evidence_pending')
    if (existingDispute) {
    ...
    // GÖREV 46: Rapor oluştur (yeni alanlarla)
    const dispute = await prisma.disputeReport.create({
      data: {
        ...
        contactEmail,
        disputeType: disputeType || type,
        expectedResolution,
        evidencePhotos: evidence || [], // Fotoğraflar evidencePhotos'a da kaydedilsin
      },
    })
    ...
    try {
      ...
      for (const admin of admins) {
        // Push notification
        sendPushToUser(admin.id, NotificationTypes.SYSTEM, {
          title: '⚠️ Yeni Anlaşmazlık Bildirimi',
          body: `${swapRequest.requester.name || 'Kullanıcı'} "${swapRequest.product.title}" için sorun bildirdi. Tür: ${disputeTypeLabels[type] || type}`,
          url: '/admin?tab=disputes'
        }).catch(err => console.error('Admin push notification error:', err))
        
        // GÖREV 46: Daha detaylı admin mesajı
      ...
      // Admin bildirimi başarısız olsa bile dispute oluşturulmuş olduğu için devam et
    }

    return NextResponse.json({
      success: true,
      disputeId: dispute.id,
      message: '✅ Anlaşmazlık bildiriminiz admin\'e iletildi. En kısa sürede incelenecektir. 48 saat içinde kanıt yükleyebilirsiniz.',
