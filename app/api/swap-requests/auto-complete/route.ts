 ...
 * ⚠️ GÖREV 24A: Transaction ile garanti altında ürün status güncelleme
 */
async function completeSwap(swap: any) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // 2. ✅ GÖREV 24A FIX: Her iki ürünü de swapped yap
    const productIds = [swap.productId]
    if (swap.offeredProductId) {
      productIds.push(swap.offeredProductId)
    }
    ...
    console.log(`✅ Takas #${swap.id} - Ürünler swapped yapıldı: ${productIds.join(', ')}`)
  ...
  // 5. Escrow'u serbest bırak (transaction dışında, hata olsa bile swap tamamlandı sayılır)
  try {
