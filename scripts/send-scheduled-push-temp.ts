...
type TemplateFunction<T = {}> = (ctx: T) => { title: string; body: string };

// Şablonlar - Her zaman dilimi için en az 3 farklı şablon
const morningTemplates: TemplateFunction[] = [
  () => ({
    title: 'Günaydın! ☀️',
...
const noonTemplates: TemplateFunction<{ count: number }>[] = [
  (ctx) => ({
...
const eveningTemplates: TemplateFunction<{ category: string; count: number }>[] = [
  (ctx) => ({
...
const nightTemplates: TemplateFunction[] = [
  () => ({
    title: 'İyi geceler! 🌙',
