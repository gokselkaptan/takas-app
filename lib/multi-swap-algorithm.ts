...
/**
 * Complete a multi-swap (mark as completed after delivery)
 */
export async function completeMultiSwap(
  multiSwapId: string,
  ...
  if (multiSwap.status !== 'confirmed') {
    throw new Error('Takas henüz onaylanmamış veya zaten tamamlanmış')
  }

  // Build participant transfer map
  // In a cycle: A gives to B, B gives to C, C gives to A
  // So A receives from C, B receives from A, C receives from B
  const participantMap: Array<{
    userId: string
    productId: string
    receivedProductId: string
  }> = []

  const participantList = multiSwap.participants
  for (let i = 0; i < participantList.length; i++) {
    const current = participantList[i]
    // In the cycle, the next person receives from current
    // So current receives from the previous person
    const previousIndex = (i - 1 + participantList.length) % participantList.length
    const previous = participantList[previousIndex]
    
    participantMap.push({
      userId: current.userId,
      productId: current.givesProduct.id,  // What this user gives
      receivedProductId: previous.givesProduct.id  // What this user receives (from previous in cycle)
    })
  }

  // Update multi-swap status to completed
  await prisma.multiSwap.update({
    where: { id: multiSwapId },
    data: { 
      status: 'completed',
  ...
  // Mark all products as swapped
  await prisma.product.updateMany({
    where: {
      id: { in: participantList.map(p => p.givesProduct.id) }
    },
    data: { status: 'swapped' }
  })
