...
// ═══ GÖREV 39: Sessiz fetch - 404 ve hataları sessizce ele al (console spam önleme) ═══
export async function safeSilentFetch<T = any>(
  url: string,
  options?: SafeFetchOptions
): Promise<T | null> {
  try {
    const result = await safeFetch<T>(url, { 
      ...options, 
      retries: 1, // Sessiz fetch'te sadece 1 retry
      timeout: options?.timeout || 8000 
    })
    
    // 404, 401, 403 hatalarında sessizce null dön (console log yapma)
    if (!result.ok) {
      return null
    }
...
// ═══ GÖREV 39: Exponential Backoff Polling Helper ═══
export interface PollingConfig {
  initialInterval: number   // Başlangıç intervali (ms) - varsayılan 30000
  maxInterval: number       // Maksimum interval (ms) - varsayılan 120000
  backoffMultiplier: number // Hata durumunda çarpan - varsayılan 2
  resetOnSuccess: boolean   // Başarılı çağrıda interval'i sıfırla - varsayılan true
}
...
export class ExponentialBackoffPoller {
  ...
  private callback: () => Promise<boolean> // true = success, false = error
  
  constructor(callback: () => Promise<boolean>, config: Partial<PollingConfig> = {}) {
    this.config = {
      initialInterval: config.initialInterval || 30000,
  ...
  async poll(): Promise<void> {
    try {
      ...
      if (success && this.config.resetOnSuccess) {
        // Başarılı - interval'i sıfırla
        this.currentInterval = this.config.initialInterval
      } else if (!success) {
        // Hata - interval'i artır (exponential backoff)
      ...
      // Hata durumunda interval'i artır
      this.currentInterval = Math.min(
        this.currentInterval * this.config.backoffMultiplier,
        this.config.maxInterval
      )
  ...
  // Interval'i manuel olarak sıfırla
  reset(): void {
    this.currentInterval = this.config.initialInterval
  }
