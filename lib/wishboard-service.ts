...
export async function findMatches(wishItemId: string): Promise<WishMatchResult> {
  ...
  // Eşleşme bulunduğunda bildirim gönder
  if (topMatches.length > 0) {
    const highScoreMatches = topMatches.filter(m => m.score >= 50);
    if (highScoreMatches.length > 0) {
      const bestMatch = highScoreMatches[0];
      const notificationTitle = bestMatch.score >= 80 
        ? '🎯 Mükemmel Eşleşme!' 
        : '🎯 Eşleşme Bulundu!';
      
      const notificationMessage = bestMatch.score >= 80
        ? `"${wish.wantTitle}" isteğinize mükemmel bir ürün eşleşti!`
        : `"${wish.wantTitle}" isteğinize ${highScoreMatches.length} ürün eşleşti`;
      ...
      try {
        await sendPushToUser(
          ...
          'wishboard_match',
          {
            ...
            requestId: wishItemId,
            productId: bestMatch.matchedProductId || undefined,
            score: bestMatch.score,
            url: `/istek-panosu?tab=my`
          }
        );
      } catch (error) {
        console.error('Wishboard match notification error:', error);
      }
    }
  }
...
/**
 * Yeni eklenen ürüne eşleşen aktif istekleri bul
 * Ürün eklendiğinde tetiklenir - mevcut wish'leri ürünle eşleştirir
 */
export async function findMatchingRequests(productId: string): Promise<{
  request: any;
  score: number;
}[]> {
  ...
  // Aktif ve süresi dolmamış istekleri al (ürün sahibi hariç)
  const activeRequests = await prisma.wishItem.findMany({
    where: {
      status: 'active',
      expiresAt: { gt: new Date() },
      userId: { not: product.userId } // Kendi isteğiyle eşleşmesin
    },
    include: { user: { select: { id: true, name: true, location: true } } }
  });
  
  const matches: { request: any; score: number }[] = [];
  
  for (const request of activeRequests) {
    let score = 0;
    
    // 1. Kategori eşleşmesi (%40)
    if (product.category?.slug === request.wantCategory || 
        product.category?.name === request.wantCategory) {
      score += 40;
    }
    
    // 2. Değer aralığı uyumu (%25)
    const productValue = product.valorPrice || 0;
    const minValue = request.wantMinValue || 0;
    const maxValue = request.wantMaxValue || Infinity;
    
    if (productValue >= minValue && productValue <= maxValue) {
      score += 25;
    } else if (request.wantMinValue || request.wantMaxValue) {
      // Değer aralığı dışında ama yakın
      const midRange = ((request.wantMinValue || 0) + (request.wantMaxValue || 1000)) / 2;
      const diff = Math.abs(productValue - midRange) / midRange;
      if (diff < 0.3) score += 10; // %30 tolerans
    }
    
    // 3. Konum eşleşmesi (%20)
    const productCity = product.city;
    if (request.preferredCity && productCity) {
      if (productCity.toLowerCase().includes(request.preferredCity.toLowerCase())) {
        score += 20;
      }
    } else if (!request.preferredCity) {
      score += 10; // Konum tercihi yoksa bonus
    }
    
    // 4. Başlık benzerliği (%15)
    const wantWords = request.wantTitle.toLowerCase().split(/\s+/).filter((w: string) => w.length > 2);
    const productWords = product.title.toLowerCase().split(/\s+/).filter((w: string) => w.length > 2);
    const commonWords = wantWords.filter((w: string) => productWords.includes(w));
    score += Math.min(15, commonWords.length * 5);
    
    if (score >= 50) {
      matches.push({ request, score: Math.min(100, Math.round(score)) });
    }
  }
