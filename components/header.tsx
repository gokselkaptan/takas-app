...
import { Menu, X, ChevronDown, User, LogOut, Package, Plus, UserPlus, Shield, Search, ArrowLeftRight, Moon, Sun, Eye, Bell, Tag, Gift, ShoppingBag, MessageSquare, MessageCircle, RefreshCw, Check, XCircle, Clock, Briefcase, Users, Sparkles } from 'lucide-react'
...
export function Header() {
  ...
  // Giriş yapılmamışken badge'i temizle
  useEffect(() => {
    if (status === 'unauthenticated') {
      setAppBadge(0) // Badge'i temizle
    }
  }, [status])
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const totalBadge = unreadMessages + pendingSwaps
  ...
  const navLinks = [
    ...
    { href: '/takas-firsatlari', label: `🔄 ${t('swapCenter')}` },
  ]
                  ...
                  <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border dark:border-gray-700 overflow-hidden z-50">
                    ...
                    <div className="p-2">
                      ...
                      <Link href="/takas-firsatlari" className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 transition-colors" onClick={() => setIsUserMenuOpen(false)}>
                        ...
                        <span className="text-sm font-medium">🔄 {t('swapCenter')}</span>
                      </Link>
            ...
            <nav className="flex flex-col gap-1">
              ...
              {status === 'loading' ? (
                ...
                <div className="bg-frozen-50/50 rounded-lg mx-2 py-2">
                  ...
                  <Link href="/mesajlar" className="flex items-center gap-2 px-4 py-3 rounded-lg hover:bg-indigo-100 text-indigo-600 font-bold" onClick={() => setIsMenuOpen(false)}>
